...
-}
generate : Encoding -> Int -> String -> Result String (Generator String)
...
regexParser =
    combine
        >> Random.Pcg.map List.concat
        >> Random.Pcg.map (List.map Char.fromCode)
        >> Random.Pcg.map String.fromList
        <$> many options
...
constat_ =
    (string2code >> Random.Pcg.constant) <$> regex "[^\\[\\]\\(\\)\\|\\?]"
...
group_ =
    Random.Pcg.choices
        <$> parens
                (sepBy (string "|")
                    (combine >> Random.Pcg.map List.concat <$> many options)
                )
    ...
    string "\\"
        *> choice
            ...
            , string "w"
                $> (words
                        |> Random.Pcg.sample
                        |> Random.Pcg.map (Maybe.withDefault 65)
                   )
            ...
            , (\enc ->
                enc
                    |> spaces
                    |> Random.Pcg.sample
                    |> Random.Pcg.map (Maybe.withDefault 32)
              )
                <$> (string "s" *> encoding)
            , (\enc ->
                ...
                <$> (string "S" *> encoding)
            , string "n" $> Random.Pcg.constant 10
            , string "t" $> Random.Pcg.constant 9
            , string "r" $> Random.Pcg.constant 13
            , string "f" $> Random.Pcg.constant 12
            , string "v" $> Random.Pcg.constant 11
            , string "b" $> Random.Pcg.constant 8
            , string "0" $> Random.Pcg.constant 0
...
filter fn encoding =
    case encoding of
        ASCII ->
            Random.Pcg.filter fn (0 ... (2 ^ 8))

        UNICODE ->
            Random.Pcg.filter fn (0 ... (2 ^ 16))
